...
int main()
{
    ...
    init(&store_partial);
	{
        ...
        #pragma omp parallel
        {
            ...
            realloc(batch_array.start,0);
            for (unsigned long i = 0 ; i < block.len ; i++) mpz_clear(*(block.start+i));
            realloc(block.start,0);
            realloc(sieve_array.start,0);
        }
